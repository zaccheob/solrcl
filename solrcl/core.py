...
from base import *
from solrtype import *
from solrfield import *
...
class MissingRequiredField(exceptions.SOLRError): pass
class DocumentNotFound(exceptions.SOLRError): pass
class SOLRReplicationError(exceptions.SOLRError): pass
...
class SOLRCore(SOLRBase):
	def __init__(self, core, domain=DEFAULT_SOLR_DOMAIN, port=DEFAULT_SOLR_PORT, blockjoin_condition=None):
		...
		if not self.blockjoin_condition is None:
			...
			try:
				res = self.select({"q": self.blockjoin_condition, "rows": 0})
	...
	def listFieldsIter(self, fields=None, limit=None, blocksize=10000, parallel=6, filter=None, sort=''):
		#Notare che i valori di default per blocksize(10000) e parallel(6) sono ottimizzati per ridurre il tempo di risposta dopo esperimenti su un server solr con 24 processori (leggiasf2)
		if fields is None:
			fields = (self.id_field,)
		if filter is None:
			query = "*:*"
